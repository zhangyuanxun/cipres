...
/*
	...
	GET $URL/job/<user>/<jobhandle>/input/<documentId>
	...
	PUT $URL/job/<user>/modify/<jobhandle>
		Alters a task. Requires at least one parameter, 'action,' whose value indicates the action
		to perform. Currently, the only supported action is to cancel a running task.
...
public class CipresJob extends BaseResource
{
	...
	*/
	...
	void initialize() throws Exception
	{
		...
		*/
		...
		if (user == null)
		{
			...
			log.debug("Setting logged in user to " + user.getUsername());
			if (user == null)
			{
	...
	public Response getInputDocument(@PathParam("id") String jobHandle, @PathParam("documentId") long documentId) throws Exception
	{
		initialize();

		try {
			TaskInputSourceDocument inputDoc = new TaskInputSourceDocument(documentId);
			Task task = inputDoc.getTask();

			if (!task.getJobHandle().equalsIgnoreCase(jobHandle))
				throw new FileNotFoundException("Document " + documentId + " not found in job " + jobHandle);

			if (task.getUserId() != user.getUserId())
				throw new UserMismatchException("User " + user.getUsername() + " doesn't own job " + jobHandle);

			if (application != null && !application.getName().equals(task.getAppname()))
				throw new UserMismatchException("Application " + application.getName() + " doesn't own job " + jobHandle);

			String fileName;

			try {
				String docName = inputDoc.getName();
				...
				fileName = inputDoc.getName();
			}

			ResponseBuilder response = Response.ok(inputDoc.getDataAsStream());
		...
		catch (NotExistException notExistErr) {
			throw new CipresException("Document Not Found Error: " + notExistErr.getMessage(), Status.NOT_FOUND, ErrorData.NOT_FOUND);
		}
	}
	...
	*/
	...
	private StatusData submit(FormDataMultiPart formData, boolean validateOnly) throws Exception
	{
		...
		String folderName = jfp.getFolderName();
		String taskLabel = jfp.getTaskLabel();
		...
		Map<String, List<UserDataItem>> inputFiles = jfp.getInputFiles();

		try
		{
			log.debug("Submitting user is " + user.getUserId() + ", " + user.getUsername());
	...
	*/
	...
	public void modifyJob(@PathParam("id") String jobHandle, MultivaluedMap<String, String> formData) throws Exception
	{
		initialize();
		...
		if (action.equalsIgnoreCase("cancel"))
			Job.cancel(jobHandle, user, this.application, false);
		else
