...
public class ${Toolname}Validator implements ParameterValidator2
{
	...
	*/
	...
	{
		Set<String> missingRequired = validateRequiredInput(input.keySet());
		for (String missing : missingRequired) 
		{
            errors.add(new FieldError(missing, "Input file parameter is required.")); 
		}
		for (Map.Entry<String, List<UserDataItem>> entry : input.entrySet())
		{
			String param = entry.getKey();
			List<UserDataItem> values = entry.getValue();
			...
			if (values.size() > 1 && !multipleAllowed.contains(param))
			{
				errors.add(new FieldError("Input file parameter " + param, " has multiple values.")); 
				continue;
			}
		}
	}
