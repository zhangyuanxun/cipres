...
public class CreateTables {
	...
	private final boolean m_dropStatements;
	...
	private CreateTables(boolean dropStatements, boolean scriptOnly)
	{
		m_dropStatements = dropStatements;
	...
	public static void main(String[] args)
	{
		try {
			...
			try {
				...
				(new CreateTables(dropStatements, scriptOnly)).create(dbConn);
			}
			finally {
				dbConn.close();
			}
	...
	{
		throw new Exception("usage: CreateTables [ --drop ] [ --script ]");
	}
	...
	private void dropTables(Connection dbConn) throws SQLException
	{
		// CASCADE has no effect in MySql
		if (DatabaseTools.getDbProductName().equals(DatabaseTools.MYSQL_DB_NAME))
			executeCommand(dbConn, "SET FOREIGN_KEY_CHECKS = 0");

		executeCommand(dbConn, "DROP TABLE IF EXISTS tgusage CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS sso CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS running_tasks_parameters CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS running_tasks CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS job_stats CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS job_events CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS cached_items CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS application_preferences CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS applications CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS task_log_messages CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS task_output_source_documents CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS task_output_parameters CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS task_input_source_documents CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS task_input_parameters CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS tool_parameters CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS task_properties CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS tasks CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS record_fields CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS data_records CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS item_metadata CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS userdata CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS source_documents CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS folder_preferences CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS folders CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS user_group_lookup CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS groups CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS user_preferences CASCADE");
		executeCommand(dbConn, "DROP TABLE IF EXISTS users CASCADE");

		if (!DatabaseTools.usesGeneratedKeys()) {
			executeCommand(dbConn, "DROP SEQUENCE IF EXISTS tgusage_tgusage_id_seq CASCADE");
			executeCommand(dbConn, "DROP SEQUENCE IF EXISTS sso_sso_id_seq CASCADE");
			executeCommand(dbConn, "DROP SEQUENCE IF EXISTS job_events_se_id_seq CASCADE");
			executeCommand(dbConn, "DROP SEQUENCE IF EXISTS cached_items_cached_item_id_seq CASCADE");
			executeCommand(dbConn, "DROP SEQUENCE IF EXISTS task_output_source_documents_output_document_id_seq CASCADE");
			executeCommand(dbConn, "DROP SEQUENCE IF EXISTS task_output_parameters_output_id_seq CASCADE");
			executeCommand(dbConn, "DROP SEQUENCE IF EXISTS task_input_source_documents_input_document_id_seq CASCADE");
			executeCommand(dbConn, "DROP SEQUENCE IF EXISTS task_input_parameters_input_id_seq CASCADE");
			executeCommand(dbConn, "DROP SEQUENCE IF EXISTS tasks_task_id_seq CASCADE");
			executeCommand(dbConn, "DROP SEQUENCE IF EXISTS data_records_record_id_seq CASCADE");
			executeCommand(dbConn, "DROP SEQUENCE IF EXISTS userdata_userdata_id_seq CASCADE");
			executeCommand(dbConn, "DROP SEQUENCE IF EXISTS source_documents_source_document_id_seq CASCADE");
			executeCommand(dbConn, "DROP SEQUENCE IF EXISTS folders_folder_id_seq CASCADE");
			executeCommand(dbConn, "DROP SEQUENCE IF EXISTS groups_group_id_seq CASCADE");
			executeCommand(dbConn, "DROP SEQUENCE IF EXISTS users_user_id_seq CASCADE");
		}

		if (DatabaseTools.getDbProductName().equals(DatabaseTools.MYSQL_DB_NAME))
			executeCommand(dbConn, "SET FOREIGN_KEY_CHECKS = 1");
}
	...
	private void createFoldersTable(Connection dbConn) throws SQLException
	{
		...
		queryBuilder.append(" NOT NULL,\n  UUID ");
		queryBuilder.append(stringTypeName);
		queryBuilder.append("(46) NOT NULL,\n  VERSION ");
		queryBuilder.append(intTypeName);
		queryBuilder.append(" DEFAULT 0 NOT NULL,\n  PRIMARY KEY (FOLDER_ID),\n  " +
				...
				"UNIQUE (UUID),\n  " +
				"UNIQUE (ENCLOSING_FOLDER_ID, LABEL)\n)");
