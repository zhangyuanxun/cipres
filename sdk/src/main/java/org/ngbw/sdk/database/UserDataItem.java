...
public class UserDataItem extends FolderItem implements SourceDocument, Comparable<UserDataItem> {
	...
	{
		super(TABLE_NAME, KEY_NAME, MAX_LABEL_LENGTH);
	...
	void delete(Connection dbConn) throws IOException, SQLException
	{
		...
		delete(dbConn, getLabel(), getUserDataId(), getSourceDocumentId());
	...
	*/
	...
	{
		Criterion userDataKey = new LongCriterion(KEY_NAME, userDataId);
		Column<Long> sourceDocumentId = new LongColumn("SOURCE_DOCUMENT_ID", false);
		Column<String> label = new StringColumn("LABEL", false, MAX_LABEL_LENGTH);
		List<Column<?>> columns = new ArrayList<Column<?>>();

		columns.add(sourceDocumentId);
		columns.add(label);

		(new SelectOp(TABLE_NAME, userDataKey, columns)).execute(dbConn);

		delete(dbConn, label.getValue(), userDataId, sourceDocumentId.getValue());
	}
	...
	private static void delete(Connection dbConn, String label, long userDataId, long sourceDocumentId) throws IOException, SQLException
	{
		Criterion nameKey = new StringCriterion("NAME", String.valueOf(userDataId));
		Column<String> nameColumn = new StringColumn("NAME", false, 255, true);

		nameColumn.assignValue(label);

		(new UpdateOp("task_input_source_documents", nameKey, nameColumn)).execute(dbConn);
