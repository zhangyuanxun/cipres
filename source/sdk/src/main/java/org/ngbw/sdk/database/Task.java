...
public class Task extends FolderItem implements Comparable<Task> {
	...
	{
		this(otherTask, otherTask.getEnclosingFolder());
	}

	public Task(Task otherTask, Folder enclosingFolder) throws IOException, SQLException
	{
		this();

		setComment(otherTask.getComment());
		setOk(otherTask.isOk());
		setTerminal(otherTask.isTerminal());
		setStage(otherTask.getStage());
		...
		setCreationDate(otherTask.getCreationDate());
		properties().putAll(otherTask.properties());
		toolParameters().putAll(otherTask.toolParameters());

		String jobHandle = "NGBW-JOB-" + getToolId() + "-" + generateUUID();

		setJobHandle(jobHandle);

		for (Map.Entry<String, List<TaskInputSourceDocument>> entry : otherTask.input().entrySet()) {
			...
			input().put(entry.getKey(), inputDocs);
		}

		for (Map.Entry<String, List<TaskOutputSourceDocument>> entry : otherTask.output().entrySet()) {
			List<TaskOutputSourceDocument> outputDocs = new ArrayList<TaskOutputSourceDocument>();

			for (TaskOutputSourceDocument doc : entry.getValue())
				outputDocs.add(new TaskOutputSourceDocument(doc));

			output().put(entry.getKey(), outputDocs);
		}

		for (Map.Entry<String, List<TaskOutputSourceDocument>> entry : otherTask.intermediateOutput().entrySet()) {
			List<TaskOutputSourceDocument> outputDocs = new ArrayList<TaskOutputSourceDocument>();

			for (TaskOutputSourceDocument doc : entry.getValue())
				outputDocs.add(new TaskOutputSourceDocument(doc));

			intermediateOutput().put(entry.getKey(), outputDocs);
		}

		for (TaskLogMessage message : otherTask.logMessages())
			logMessages().add(new TaskLogMessage(message));
	}
