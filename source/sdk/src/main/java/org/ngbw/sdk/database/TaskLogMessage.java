...
public class TaskLogMessage extends Row implements Comparable<TaskLogMessage> {
	...
	{
		this(dbConn, taskId, messageIndex, getJobHandle(dbConn, taskId));
	}

	TaskLogMessage(TaskLogMessage otherMessage) throws IOException, SQLException
	{
		this();

		setStage(otherMessage.getStage());
		setError(otherMessage.isError());
		setMessage(otherMessage.getMessage());
		setJobHandle(otherMessage.m_jobHandle);

		m_creationDate.setValue(otherMessage.getCreationDate());
	}

	private TaskLogMessage(Connection dbConn, long taskId, int messageIndex, String jobHandle) throws IOException, SQLException
	{
		this();
	...
	static List<TaskLogMessage> findLogMessages(long taskId) throws IOException, SQLException
	{
		...
		try {
			...
			String jobHandle = getJobHandle(dbConn, taskId);
			List<TaskLogMessage> messages = new ArrayList<TaskLogMessage>();

			while (messageRows.next())
				messages.add(new TaskLogMessage(dbConn, taskId, messageRows.getInt(1), jobHandle));
	...
	private static String getJobHandle(Connection dbConn, long taskId) throws IOException, SQLException
	{
		Criterion key = new LongCriterion("TASK_ID", taskId);
