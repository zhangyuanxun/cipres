...
public class SessionManager extends NgbwSupport
{
	...
	private static final Pattern usernamePattern = Pattern.compile("^[a-zA-Z0-9_-]{5,16}$");
	/*
	...
	*/
	...
	private void storeIplantHeaders()
	{
		...
		eppn = request.getHeader("PRINCIPAL");
		if (eppn == null || eppn.trim().equals(""))
		{
			return;
		}
		eppn += "@iplantcollaborative.org";

		uid = request.getHeader("uid");
		mail = request.getHeader("CAS_email");
		givenName = request.getHeader("CAS_firstName");
		sn = request.getHeader("CAS_lastName");
		if (uid == null)
		{
			uid = "";
		}
		...
		*/
		logger.debug("Stored iplant headers eppn=" + shibHeaders.getProperty("eppn") + ", uid=" + shibHeaders.getProperty("uid") + ", mail=" +
	...
	*/
	...
	public String input()
	{
		...
		try
		{
			...
			if (cipresUser != null)
			{
				logger.debug("this iplant user has a linked cipres account");
				String retval = iplant2CipresLogin(cipresUser);
				return retval;
			}
			logger.debug("this iplant user doesn't have a linked cipres account. SessionManager.input is returning " + IPLANT_REGISTER);
	...
	*/
	...
	{
		// TL: added 6_2017, working on CAS login.
		dumpRequestHeaders("iplantLogin");
	...
	*/
	...
	public String logout() {
		// try block is just for debugging.
		try
		{
 	...
 	// This is CAS specific.
    private void iplantLogout()
    {
        Cookie[] cookies = this.request.getCookies();
        for (Cookie cookie : cookies)
        {
            if (cookie.getName().equalsIgnoreCase("MOD_AUTH_CAS"))
            {
                logger.debug("Have cookie MOD_AUTH_CAS, deleting it in response. " );
                cookie.setMaxAge(0);
                response.addCookie(cookie);
            }
            if (cookie.getName().equalsIgnoreCase("MOD_AUTH_CAS_S"))
            {
                logger.debug("Have cookie MOD_AUTH_CAS_S, deleting it in response. " );
                cookie.setMaxAge(0);
                response.addCookie(cookie);
            }
        }
	...
	protected boolean validateUser() {
		...
		{
			String confirm = getConfirmNewPassword();
			if (confirm == null || !password.trim().equals(confirm.trim()))
			{
		...
		{
			String confirm = getConfirmEmail();
			if (confirm == null || !email.trim().equals(confirm.trim()))
			{
